...
abstract class AbstractBaseFixture extends Fixture
{
     ...
     */
    protected function createMany(int $count, string $className, callable $factory): void
    {
        for ($i = 0; $i < $count; ++$i) {
            $entity = $factory($i);

            if (null === $entity) {
                throw new LogicException('Did you forget to return the entity object from your callback to AbstractBaseFixture::createMany()?');
            }
    ...
    protected function getRandomReference(string $className): object
    {
        ...
        if (empty($this->referencesIndex[$className])) {
            throw new InvalidArgumentException(sprintf('Did not find any references saved with the class name "%s"', $className));
        }
